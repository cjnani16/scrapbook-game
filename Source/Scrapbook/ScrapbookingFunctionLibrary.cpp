...
FDialogueData UScrapbookingFunctionLibrary::CreateRoundtableDialogue(
    const TArray<FJurorData>& Jurors,
    const TArray<FName>& JurorNames,
    const TArray<FEvidenceTrait>& Traits)
{
    ...
    const TArray<FString> NoImpactTemplates = {
        ...
        TEXT("I still don't see what this proves."),
    ...
    for (int32 i = 0; i < NumJurors; ++i)
    {
        ...
        {
            FName TraitName = Traits[j].TypeName;
            if (const float* MultiplierPtr = Jurors[i].TraitMultipliers.Find(TraitName))
            {
                // A trait is salient if it has a non-zero multiplier
                if (FMath::Abs(*MultiplierPtr) > 0.01f)
                {
                    SalientTraitIndices.Add(j);
                    SalientTraitMultipliers.Add(*MultiplierPtr);

                    // Build Progression Fact string: [JurorName][TraitName]
                    FString FactString = FString::Printf(TEXT("%s%s"), *JurorNames[i].ToString(), *TraitName.ToString());
                    FName FactName = FName(*FactString);

                    // Check if it exists to avoid overwriting or setting twice
                    bool bFound = false;
                    UScrapbookingFunctionLibrary::GetProgressionFactValue(FactName, bFound);

                    if (!bFound)
                    {
                        // Add to the Line's progression facts so the dialogue system sets it when this line plays
                        LineData.ProgressionFactsToSet.Add(FactName, TEXT(""));
                    }
                }
            }
        }

        FString FinalLineText;

        // 2. Select template and populate placeholders if we have salient traits
        if (SalientTraitIndices.Num() > 0)
        {
            // Pick a random salient trait to focus on for this line
            int32 RandomIndex = FMath::RandRange(0, SalientTraitIndices.Num() - 1);
            int32 TraitIdx = SalientTraitIndices[RandomIndex];
            float MultiplierMagnitude = FMath::Abs(SalientTraitMultipliers[RandomIndex]);
            ...
            FString TraitFinal = FString::Format(TEXT("<bold>{0}</>"), { SelectedTraitName.ToLower() });
            FinalLineText = ChosenTemplate.Replace(TEXT("[TRAITNAME]"), *TraitFinal, ESearchCase::IgnoreCase);
        }
        else
        {
        ...
        DialogueData.Lines.Add(LineData);
    }

    return DialogueData;
}

void UScrapbookingFunctionLibrary::SaveJurorHint( const UObject* WorldContextObject, FName JurorName, FString HintText)
{
    FName HintFact = FName( JurorName.ToString() + "Hints" );
    bool Found;
    FString PrevHints = GetProgressionFactValue( HintFact, Found );
    if (!PrevHints.IsEmpty())
    {
        PrevHints.Append("|");
    }
    PrevHints.Append(HintText);
    SetProgressionFactValue( WorldContextObject, HintFact, PrevHints);
}

TArray<FString> UScrapbookingFunctionLibrary::GetJurorHints( FName JurorName )
{
    ...
    if (!PrevHints.IsEmpty())
    {
        PrevHints.ParseIntoArray( Hints, TEXT("|") );
    }
    return Hints;
}
